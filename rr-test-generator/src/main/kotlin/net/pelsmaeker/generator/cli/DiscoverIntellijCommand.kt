...
class DiscoverIntellijCommand: CliktCommand(
    ...
    override fun run() {
        ...
        for (javaProject in javaProjects) {
            ...
            if (path != null) {
                Cli.info("  ${javaProject.directory}/${javaProject.name}${javaProject.qualifier?.let { "_$it" } ?: ""}")
            } else {
        ...
        if (skippedProjects.isNotEmpty()) {
            ...
            for (skippedProject in skippedProjects) {
                Cli.warn("  ${skippedProject.directory}/${skippedProject.name}${skippedProject.qualifier?.let { "_$it" } ?: ""}")
            }
        }
